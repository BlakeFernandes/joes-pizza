...
const shopCommand: SlashCommand = {
  ...
  execute: async (interaction) => {
      ...
      if (option === "shop") {
        ...
        for (const shop of shops) {
          ...
          const price = Math.round(
            shop.price *
              Math.pow(shop.priceExponent, userShop?.amountOwned ?? 1)
          );
      ...
      if (option === "buy") {
        ...
        const price = Math.round(
          shop.price * Math.pow(shop.priceExponent, amountOwned)
        );
        ...
        if (price > currentBalance) {
          ...
          await interaction.reply(
            `Insufficient funds. You need $${Math.round(
              missingAmount
            )} more to buy ${shop.name}.`
          );
          return;
        }
        ...
        await interaction.reply(
          `You bought a ${shop.name} for $${Math.round(price)}.`
        );
      }
  },
};
